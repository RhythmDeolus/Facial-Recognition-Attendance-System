<template>
    <div :key="rerenderkey" class="relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="item in computedCalendars" :key="item[0]"
...
let computedCalendars = computed(() => {
    let term = search.value.toLowerCase();
    if (term)
    return calendars.value.filter((calendar) => {
        return calendar[0].toLowerCase().includes(term) ||
        calendar[1].toLowerCase().includes(term);
    });
...
async function fetchAttendanceData() {
    try { 
        let calendarData = await useFetch('/api_1/get_academic_calendars', {
        semester_id: semester_id
        }, 'GET');
        console.log(calendarData);
        setTable(calendarData);
        rerender()
    } catch (error) {
        console.error(error);
    }
...
function setTable(calendarData) {
    calendars.value = [];
    let calendarIds = Object.keys(calendarData);
