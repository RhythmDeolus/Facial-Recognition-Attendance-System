<template>
    <div class="max-w-md mx-auto">
        ...
        <div :key="rerenderCalendarKey" class="relative z-0 w-full mb-5 group">
            <label for="calendars" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select a calendar</label>
            <select v-model="calendar_id" id="calendars" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option v-for="calendar_id in Object.keys(calendars)" :value="calendar_id">{{calendar_id}}</option>
            </select>
        </div>
        <div :key="rerenderSemesterKey" class="relative z-0 w-full mb-5 group">
            ...
            <select @change="get_calendars()" v-model="semester_id" id="semesters" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
...
let rerenderCalendarKey = ref(0);

function rerenderCalendars() {
    rerenderCalendarKey.value += 1;
}
...
function get_calendars() {
    ...
    useFetch('/api_1/get_academic_calendars', {
...
function checkForm(e) {
    if (Number(calendar_id) && Number(course_id) 
        && Number(semester_id) && Number(semno)) {
        useFetch( '/api_1/assign_calendar_course', {
            'course_id': course_id,
